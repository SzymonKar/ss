- okreslanie dnia, godziny, czasu trwania i trenera,
- zapisywanie uzytkownikow na zajecia,
- zarzadzanie harmonogramem.

### Profil i postepy

Uzytkownik moze prowadzic profil oraz zapisywac dane zwiazane z forma, np. wage i wymiary ciala. Dane te sa przechowywane w tabelach profilu i postepow.

### Sklep

Modul sklepu zawiera:

- produkty,
- zamowienia,
- pozycje zamowien,
- status zamowienia,
- laczenie zamowien z uzytkownikiem.

### Trening w plenerze

Aplikacja posiada endpoint pogodowy:

```text
GET /api/weather/outdoor-gym?latitude=52.2297&longitude=21.0122
```

Endpoint pobiera pogode z Open-Meteo i zwraca rekomendacje dotyczaca treningu na zewnatrz.

## 4. Struktura projektu

Glowna aplikacja znajduje sie w katalogu:
...
- `Pages/` - strony Razor Pages widoczne w aplikacji.
- `Models/` - modele domenowe, np. plan treningowy, cwiczenie, dieta, produkt, zamowienie.
- `DTOs/` - obiekty do przenoszenia danych.
- `Data/` - konfiguracja bazy danych i seed ról/uprawnien.
- `Repositories/` - warstwa dostepu do danych.
- `Security/` - role, polityki i uprawnienia aplikacji.
- `Weather/` - integracja z Open-Meteo i logika rekomendacji treningu w plenerze.
- `Migrations/` - migracje Entity Framework Core.
- `wwwroot/` - pliki statyczne, style, skrypty, biblioteki frontendowe i uploady.
